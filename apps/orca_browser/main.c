...
static const uint8_t cat_count[4]={20,43,2,6};
...
wipe();box(lv_screen_active(),0,0,480,48,lv_color_hex(0x34343A));label(lv_screen_active(),"ORCA / FIELD NOTES",16,9,260,&lv_font_montserrat_20,CREAM);label(lv_screen_active(),"71 DOCUMENTED STORIES",17,32,220,&lv_font_montserrat_14,TEAL);lv_obj_t *logo=button(lv_screen_active(),"",336,4,132,42,site_click,NULL);lv_obj_set_style_bg_color(logo,lv_color_hex(0x34343A),0);lv_obj_set_style_border_width(logo,0,0);lv_obj_set_style_pad_all(logo,0,0);lv_obj_t *mark=lv_image_create(logo);lv_image_set_src(mark,&freewili_logo);lv_obj_center(mark);
